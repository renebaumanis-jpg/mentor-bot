...
MEMORY REMINDERS: When Renē shares something important mid-conversation — a goal, a commitment, a key insight, a win — naturally remind him to save it. Keep it casual, like: "Save that — /remember I want to hit €1k/month by August" or "That's worth keeping, type /remember [thing]." Don't do it every message, only when something genuinely matters."""

def build_system(user_id, extra=""):
    profile = get_profile(user_id)
    mems = get_memories(user_id, limit=20)
    patterns = get_patterns(user_id, limit=6)
    s = BASE_PERSONA
    if profile:
...
# ---------------- CLAUDE ----------------
async def call_claude(system, messages, max_tokens=500):
    try:
...
async def handle_message(update: Update, context):
    ...
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
    msgs = get_recent_messages(uid, limit=12)
    reply = await call_claude(build_system(uid), msgs)
    save_message(uid, "assistant", reply)
    await update.message.reply_text(reply)
    # Only extract memory every 4th message to cut API costs — not every single exchange needs it
    msg_count = len(get_recent_messages(uid, limit=1000))
    if msg_count % 8 == 0:  # roughly every 4 user messages (user+assistant pairs)
...
async def send_weekly_review(bot, uid, chat_id):
    ...
    msgs.append({"role": "user", "content": instr})
    reply = await call_claude(build_system(uid), msgs, max_tokens=700)
    save_message(uid, "assistant", reply)
